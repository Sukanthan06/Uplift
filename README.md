...
All 8 phases complete. See `ARCHITECTURE.md` for system design, `docs/DECISIONS.md` for every non-obvious choice (including every bug a live test caught before shipping), `docs/ASSUMPTIONS.md` for the full frozen assumption list, and `docs/DEMO_SCRIPT.md` for the phase-by-phase demo walkthrough.

### The headline result, honestly

The learned uplift model **ties, doesn't beat**, a simple hand-picked rule
(−0.5% at matched budget on held-out test data) — reported as-is, not spun.
What *does* hold up: even before any ML exists, targeting and timing beat
brute-force retrying (rule-based recovers 92% of retry-3x's revenue at 36%
of the retries), and the sensitivity sweep suggests the model's gap is a
training-data-volume limit, not a structural flaw — it narrows from −25% to
near-parity as failure volume increases. Full numbers below.

## Quickstart

```bash
cp .env.example .env
# edit .env: set GROQ_API_KEY if you want the diagnoser (Phase 5+) to make
# real LLM calls. Phases 1-4 don't need it.

docker compose up -d --build
```

Then, inside the running backend container (packages are `pip install`ed
directly into it, no venv indirection — just run these as-is):

```bash
docker compose exec backend alembic upgrade head            # create the 5 tables
docker compose exec backend python -m simulator.generator   # 50k payment attempts, ~6s
docker compose exec backend python -m ml.train_uplift        # T-learner, a few seconds
docker compose exec backend python -m ml.evaluate             # Phase 2 baseline table
docker compose exec backend python -m ml.sensitivity_sweep    # Phase 4 sweep, ~2-3 min (optional, feeds the dashboard's Phase 4 section)
docker compose exec backend python -m app.demo_pipeline       # Phase 5/6 live demo (needs GROQ_API_KEY)
```

Open `http://localhost:5173` for the dashboard, `http://localhost:8000/health`
for the API, `http://localhost:8000/audit/verify` for the raw audit endpoint.

Working locally instead of through Docker (`backend/`, `uv sync --extra
dev`): the same commands with a `uv run` prefix and `DATABASE_URL` pointed
at the host-mapped port instead of the container-internal one —
`postgresql+psycopg://uplift:uplift@localhost:5433/uplift` (already the
default in `.env.example`).

Run the test suite (no DB or network needed — the whole suite is
dependency-injected/in-memory except the scripts above):
...
### Phase 2 — baseline comparison (honest, no ML yet)

`python -m ml.evaluate`, on the 50k simulated attempts (6,693 failed):
...
The hand-picked rule-based policy (skip hard declines, time retries by cause) recovers 92% of retry-3x's revenue using 36% of the retries. Targeting and timing beat brute force even before any model exists.

### Phase 3 — uplift model (honest — this is the actual result, not a rehearsed win)
