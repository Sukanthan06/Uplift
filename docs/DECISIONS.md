...
## Pre-submission audit — `random_state` seeded on the T-learner's XGBoost models

**Chosen:** `ml/train_uplift.py`'s `_fit()` now passes `random_state=RANDOM_SEED` (42, matching `sim_config.yaml`'s `random_seed`) to both the control and treated `XGBClassifier`.

**Why:** the simulator itself was correctly seeded from Phase 1, but neither model set a seed, so `python -m ml.train_uplift` produced a slightly different Phase 3 result every time it ran -- the reported ₹599,005 / −0.5% figure from the entry above was one particular draw, not a reproducible number. Caught while re-verifying the project end to end for submission: two consecutive retrains against the same data now produce bit-for-bit identical output (₹592,384 / −1.6%, `uplift@20%=0.78`). The story (ties, doesn't beat, rule-based; gap narrows with more data in the sensitivity sweep) is unchanged -- only the exact rupee figure was ever non-deterministic, and every doc citing the old figure (`README.md`, `docs/ASSUMPTIONS.md`, `docs/DEMO_SCRIPT.md`, this file) has been updated to the new, reproducible one.

**Also fixed in the same pass:** `app/db.py` built a real SQLAlchemy engine at *module import time* (`create_engine(get_settings().database_url)` ran the instant anything did `import app.db`), which meant no test could import the module to inject a fake session without a live `DATABASE_URL` configured. Both the engine and the sessionmaker are now built lazily and cached; `engine` itself (used directly by `simulator/generator.py`) is exposed via a PEP 562 module `__getattr__` so it stays lazy too. This, plus a hand-rolled `FakeSession` test double (`tests/conftest.py`) that supports exactly the query shapes this codebase's services issue, brought `execute_retry()`, `audit.append()`/`verify()`, and `run_pipeline()` up from zero unit test coverage (previously exercised only live, via `demo_pipeline.py`) without needing a live Postgres connection for any of it.

**A second real bug caught by the new tests, not by reading the code**: logging `audit_append_finished` with a field literally named `event=` collides with structlog's own positional `event` argument and raises `TypeError` on every single call -- which would have broken every stage of the real pipeline the moment structured logging went live, since every `audit.append()` payload in `pipeline.py` carries an `"event"` key. Renamed the log field to `audit_event`. Same category as every other bug this project has caught: invisible to a hand-picked unit test fixture, immediate the moment something exercised the real call shape.

## Pre-submission audit — frontend: sidebar navigation, and a Docker/Windows HMR trap

**Chosen:** the dashboard's top tab bar became a sidebar (four destinations crosses the four-or-more-pages threshold this project's own visual audit checklist sets), collapsing to a horizontally scrollable strip on mobile rather than a full drawer. Audit Verify's chain status became the page's largest element (a banner, not one tile among three) and its record table gained payment/event/hash columns -- both needed real, small backend additions (`AuditRecordResult` and the `/audit/verify` response now carry `event`/`order_id`/`hash` per record, previously only `id`/`valid`/`reason`) since the data already existed server-side and just wasn't surfaced.

**Why the Batch Run size options are 5/10/25/50, not the audit spec's suggested 100/1000/5000:** the API caps `limit` at 50, and for good reason -- every attempt makes a real Groq call (roughly 1.2s each, observed live) plus real Postgres writes; 1000 attempts would take on the order of 20 minutes of live LLM traffic for a page meant to demo in seconds. Matched the picker to what the pipeline can actually do live, not to the spec's assumed scale.

**A real bug, not a code-reading catch**: verifying a `format.ts` fix (timestamps rendering as "30 Aug 2026, 11:04 PM IST", not `en-IN`'s "4 Sept ... am") in a browser showed the *pre-edit* output even after a hard reload. Curling the dev server's served module confirmed it was still transforming the old file content, while `docker compose exec frontend cat` confirmed the bind-mounted file on disk was already correct. Root cause: chokidar (Vite's file watcher) never receives native filesystem events for a Windows-host bind mount landing in a Linux container, so it silently keeps serving whatever it last read at container start -- no error, no stale-content warning, just wrong output that looks exactly like a code bug. Fixed with `server.watch.usePolling` in `vite.config.ts`. Logged here because it would otherwise cost real time again the next session someone edits this frontend inside Docker on Windows and can't figure out why their change "isn't showing up."
